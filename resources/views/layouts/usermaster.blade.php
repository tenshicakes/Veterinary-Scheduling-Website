...
<body class="bg-background text-gray-800 font-sans" x-data="{ sidebarOpen: false }">
    ...
    <div :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-72 bg-white shadow-[8px_0_30px_rgba(0,0,0,0.12)] transform transition-transform duration-300 flex flex-col">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            <a href="{{ route('user.home') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-lg {{ request()->routeIs('user.home') ? 'bg-blue-50 text-primary' : 'text-primary hover:bg-blue hover:text-white' }} transition">
                <!-- Lucide Home Icon -->
            ...
            <a href="{{ route('user.appointment') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-lg {{ request()->routeIs('user.appointment') ? 'bg-blue-50 text-primary' : 'text-primary hover:bg-blue hover:text-white' }} transition">
            ...
            <a href="{{ route('user.profile') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-lg {{ request()->routeIs('user.profile') ? 'bg-blue-50 text-primary' : 'text-primary hover:bg-blue hover:text-white' }} transition">
                <!-- Lucide User Icon -->
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <a href="{{ route('user.services') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-lg {{ request()->routeIs('user.services') ? 'bg-blue-50 text-primary' : 'text-primary hover:bg-blue hover:text-white' }} transition">
                <!-- Lucide User Icon -->
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-info preview-icon"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>
                Services
            </a>
        </nav>
